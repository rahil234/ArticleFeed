...
export default function EditArticlePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsSaving(true)
    try {
      await articleApi.update(articleId, formData)
      toast({
        title: "Success",
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mt-2">
                  {formData.images.map((image) => (
                    <div key={image} className="relative aspect-video rounded-lg overflow-hidden bg-muted">
                      <img src={image || "/placeholder.svg"} alt="Article" className="h-full w-full object-cover" />
                      <Button
                        type="button"
                        size="icon"
                        variant="destructive"
